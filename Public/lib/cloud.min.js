...
function generate() {
    ...
    var vMax = 250
    layout.font(font).spiral(spiral), fontSize = d3.scale[scale]().range([10, 100]), tags.length && fontSize.domain([+tags[tags.length - 1].value || 1, +tags[0].value]), complete = 0, statusText.style("display", null), words = [], layout.stop().words(tags.slice(0, max = Math.min(tags.length, +vMax))).start()
}
...
// width and height of the wordcloud svg is defined here
var fill = d3.scale.category20b(),
    ...
    layout = d3.layout.cloud()
        ...
        .on("word", progress)
        .on("end", draw),
    echoForm = d3.select("body")
        .append("form")
        .attr("action", "https://www.jasondavies.com/echo")
        .attr("target", "_blank")
        .attr("method", "POST"),
    echoContentType = echoForm.append("input")
        .attr("type", "hidden")
        .attr("name", "content-type"),
    echoInput = echoForm.append("input")
        .attr("type", "hidden")
        .attr("name", "echo")
    ...
    .append("svg")
    .attr("width", w)
    .attr("height", h), background = svg.append("g"), vis = svg.append("g")
...
parseText(d3.select("#text").property("value")), d3.select("#random-palette").on("click", function() {
    ...
    function() {
        ...
        function e() {
            ...
            s.enter().append("path").attr("class", "drag").attr("d", "M-9.5,0L-3,3.5L-3,-3.5Z")
